<div>
    ...
    <div class="comment-form">
        <form>
            ...
            <textarea id="komentar" name="komentar" wire:model='komentar_balasan' required></textarea>

            @if ($isEdit)
            <button type="button" wire:click='update()'>Update Komentar</button>
            @else
            <button type="button" wire:click='store_reply_comment()'>Kirim Komentar</button>
            @endif
        </form>
    </div>
    @foreach ($replies_comments as $reply)
        <div class="comment">
            <div class="comment-header">
                <span class="comment-author">{{ $reply->user->name }}</span>
                <div class="comment-actions">
                    @if ($reply->isLikeReplyComment(Auth::user()->id))
                        <svg class="like-icon" style="color: red;" xmlns="http://www.w3.org/2000/svg" width="16"
                            height="16" fill="currentColor" class="bi bi-heart" viewBox="0 0 16 16"
                            wire:click='like_reply_comment({{ $reply->id }})'>
                            <path
                    ...
                    @else
                        <svg class="like-icon" xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                    ...
                    @endif
                    <span class="like-count">{{ $reply->countLikeReplyComment() }}</span>
                    <!-- untuk pemilik komentar -->
                    @if ($reply->user->id === Auth::user()->id)
                        <!-- untuk pemilik komentar -->
                        <?xml version="1.0" encoding="utf-8"?>
                        <svg width="20px" height="20px" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg" wire:click='delete_confirmation({{ $reply->id }})'>
                            <path
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="15px" height="15px"
                            viewBox="0 0 50 50" wire:click='edit({{ $reply->id }})'>
                            <path
            ...
            <p class="comment-text">{{ $reply->komentar }}</p>
        </div>
    @endforeach
    <style>
        /* Style form komentar */
